...
- **Highlight элементов виден и не пропадает мгновенно.** Рамка ставилась
  обычным inline-стилем (`el.style.outline = ...`), поэтому `outline` сайта с
  `!important` (частый случай на inputs/ссылках) переопределял цвет, и рамка
  рендерилась невидимой. Теперь `outline` ставится через
  `setProperty(..., 'important')` и переживает `!important`-стили сайта.
  Кроме того рамка снималась в конце того же действия (в `finally`), из-за
  чего для быстрых действий не успевала проявиться (в т.ч. из-за
  CSS-transition цвета) и не попадала на screencast. Теперь подсветка
  «залипает» на последнем затронутом элементе и снимается только перед
  следующим действием - document-wide по маркеру `data-tq-highlight`, так что
  переживает навигацию и перерендер (устаревший элемент просто не находится).
...
- **`BiDiBrowserActions` + `BiDiElementActions`** - высокоуровневые обёртки над BiDi-API, по аналогии с `JsActions` / `ElementJsActions`. Получают driver через `driver_getter`-композицию от `BrowserService` (`browser.bidi`, `element.bidi_actions`), а не через container-lookup. Экспонируют raw BiDi-модули (`script` / `network` / `browsing_context` / `input` - типизированы как `Script`/`Network`/`BrowsingContext`/`Input`) - escape hatch для кастомных интеракций, по аналогии с `element.wait.until(custom_condition)`. Стартовый набор обёрток: `intercept_file_dialog([path])` (context-manager перехват file-dialog без JS-инъекции, Chrome/Firefox/Edge), `capture_screenshot()` (viewport + element-clipped через `browsingContext.captureScreenshot`).
- **`Actions` - W3C input-actions chain** на `BaseElement` (не raw `WebElement`). `browser.actions.move_to(menu).click(submenu).send_keys("hello").perform()` - fluent builder со всеми W3C-методами (mouse / keyboard / scroll / pause). `_find()` откладывается до `perform()` - stale-reference между построением и выполнением исключён. Каждый `browser.actions`-вызов отдаёт свежий builder.
- **`BiDiLogCollector` + LogsConfig** - сбор browser console / network логов через BiDi с автокреплением к шагам и тесту. `SeleniumConfig.logs.network` / `logs.js`: `mode` (`never` / `on_fail` / `always`), `filters` (whitelist regex), `ignored_patterns` (blacklist regex, whitelist priority), `size_limit` (байты, truncate с маркером). `BiDiLogCollector` подписывается через `BiDiBrowserActions`-фасад (`subscribe_to_events(browser.bidi)`), а step-hook'и через `install_step_hooks(logger, cfg.logs)` снимают `now()` на enter и сливают окно на exit. Тесты покрывают precedence-правила фильтрации (`[]` и `[".*"]`), регексы по URL / headers / body / status, real-world сценарии (drop static assets, keep API errors only, PII-blacklist).
- **`ShadowRootProxy` - lazy-обёртка над Shadow DOM** хост-элемента. `BaseElement.shadow_root` теперь возвращает прокси, а не сырой `selenium.webdriver.remote.shadowroot.ShadowRoot`: typed getter'ы (`get_button` / `get_input` / `get_label` / `get_checkbox` + generic `get_element[E: BaseElement]`) строят `BaseElement`-инстансы со shadow-aware `_find`. На каждом `_find()` цепочка `parent_find().shadow_root.find_element(by)` пересобирается - stale reference исключён, цепочки произвольной глубины (`form.shadow_root.get_label(...).shadow_root.get_button(...)`) резолвятся при финальном вызове.
- **Типизированная геометрия элемента**: `BaseElement.rect` / `.size` / `.location` - `NamedTuple`-ы `ElementRect(x, y, width, height)` / `ElementSize(width, height)` / `ElementLocation(x, y)` вместо `dict[str, Any]`. Поля - `float` (Firefox даёт sub-pixel для DPR > 1); tuple-unpack совместим: `x, y, w, h = element.rect`.
- **`BaseElement` - покрытие WebElement-API**: `tag_name`, `is_selected`, `aria_role`, `accessible_name` (properties), `get_property(name)`, `get_dom_attribute(name)`, `value_of_css_property(name)`, `screenshot()` (PNG bytes через классический WebDriver - BiDi-вариант через `element.bidi_actions.capture_screenshot()`).
- **Core: `Logger.register_step_enter_hook` / `register_step_exit_hook`** - per-Logger hook-mechanism для шагов. Hook-стек, `current_step` / `active_step_stack` - тоже per-Logger, не модульный state: в параллельных прогонах `pytest-xdist` каждый тест получает свой Logger со своими хуками, не путая «innermost step какого из тестов». Невыставленные хуки - non-breaking, существующий код шагов не меняется.

### Изменено

- **Top-level exports**: `Actions`, `BiDiBrowserActions`, `BiDiElementActions`. `from tquality_selenium import ...`.
- **Композиция через `driver_getter` вместо container-lookup** в `JsActions` / `ElementJsActions` / `BiDiBrowserActions` / `BiDiElementActions` / `BiDiLogCollector`. Раньше каждый класс резолвил драйвер через `SeleniumServices.get_service(BrowserService).driver`; теперь принимают `driver_getter: Callable[[], WebDriver]` в `__init__` и обращаются через него. `BrowserService` композирует фасады лениво (`browser.js_actions` / `browser.bidi` / `browser.actions`); `BaseElement` передаёт `driver_getter=lambda: self._browser.driver`. DI-singleton'ы `js_actions` и `bidi_browser_actions` удалены из `SeleniumServices` - composition пришла на их место.
- **Module-level helper'ы свёрнуты на классы**. `collection_factory.py`: `_annotation_is_webelement` / `_annotation_base_element_type` - `@staticmethod` на `CollectionFactory`, вызов через `cls.` сохраняет polymorphism. `browser.py`: семь helper'ов (`_find_chrome_binary`, `_selenium_manager_chromedriver`, `_copy_chromedriver_to_own_cache`, `_enable_bidi`, `_apply_linux_docker_chromium_flags`, `_ensure_chromedriver_runnable`) - `@staticmethod` на `BrowserService`, вызовы `self.X(...)` дают subclass-override. `is_browser_started()` - `BrowserService.is_started()` classmethod; ContextVar `_browser_started` - class attribute `BrowserService._started`.
- **Покрытие `Any` → конкретными типами**. `_driver: WebDriver`, `_log: Logger`, `_browser: BrowserService`, BiDi raw-module свойства `script: Script` / `network: Network` / `browsing_context: BrowsingContext` / `input: Input`, `opts: ArgOptions` (common base browser-options), `pytest_runtest_makereport() -> Generator[None, Any, None]`, DI-резолверы → `WebDriver` / `Logger`, `screencast_provider` `bc: BrowsingContext` / `cdp: Callable[..., Any] | None`. Где `Any` остался: pass-through `*args/**kwargs`, `execute_script`-return (`Any` у самого Selenium), `DomField` factory return (Pydantic convention), BiDi event-converters (defensive multi-shape).
